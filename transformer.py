...
def prepare_encoder_input(src_wids, src_masks, lang, params, forward=None, backward=None):
    ...
    #src_vocab_size = sum(params["src_vocab_size"].values())
    ...
    with tf.variable_scope('WordEmbedding'):
      if params['init_emb']:
        if lang not in inited_emb:
            ...
            with open(params['init_emb'][lang], 'r') as file_in:
                ...
                assert vec_len == hidden_size, "init emb size mismatched"
                #for line in file_in:
                #    line = line.split()
                #    if len(line) != vec_len + 1:
                #        tf.logging.info('skip line')
                #        continue
                ...
                #    else:
                #        idx = 1
                #    #idx = vocab_dict.get(line[0], 1)
                #    init_emb[idx, :] = [float(x) for x in line[1:]]
      ...
      #langs, v_sizes = zip(*params["src_vocab_size"].items())
      #emb_split = tf.split(emb_all, v_sizes)
      #lang_emb = {}
      #for idx, ll in enumerate(langs):
      #  lang_emb[ll] = emb_split[idx]
      ## shape: [batch_size, max_length, hidden_size]
      #emb = tf.gather(lang_emb[lang], src_wids)
    ...
    if forward is True:
        ...
        # encoder_self_attention_bias_f = encoder_self_attention_bias_f + encoder_self_attention_bias
    else:
    ...
    if backward is True:
        ...
        #encoder_self_attention_bias_b = encoder_self_attention_bias_b + encoder_self_attention_bias
    else:
...
def prepare_mask(embedding, directional=None):
    if directional == 'forward':
        encoder_self_attention_bias = common_attention.attention_bias_lower_triangle(tf.shape(embedding)[1])
    else:
        encoder_self_attention_bias = common_attention.attention_bias_higher_triangle(tf.shape(embedding)[1])
    return encoder_self_attention_bias
...
def transformer_encoder(encoder_input,
    ...
    with tf.variable_scope(name):
        ...
        ## @hr
        if lang is not None and 'lang_spec_layer' in params and params['lang_spec_layer'] is not None:
            with tf.variable_scope("layer_{}".format(lang)):
                if params['lang_spec_layer'] == 'ffn':
                    tf.logging.info('Add {} specific ffn layer'.format(lang))
                    o = transformer_ffn_layer(temp, params)
                    x = layer_process(x, o, postproc_actions, prepost_dropout)
                    x = tf.multiply(x, mask)

                    temp = layer_process(None, x, preproc_actions, prepost_dropout)
                    hidden_tensor.append(temp)
                    if 'align_lang_spec' in params and params['align_lang_spec']:
                        tf.logging.info('Align language specific layer')
                        hidden_align.append(temp)
                elif params['lang_spec_layer'] == 'block':
                    tf.logging.info('Add {} specific block layer'.format(lang))
                    o, w = common_attention.multihead_attention(
                        # layer_process(None, x, preproc_actions, prepost_dropout),
                        temp,
                        None,
                    ...
                    tf.logging.info('layer type not supported')
                    sys.exit()
        return temp

def transformer_both_encoder(encoder_input,
                        encoder_self_attention_bias,
                        mask,
                        params=None,
                        name="encoder"):
    """
        only use for both direction
    """
    num_hidden_layers = params["num_hidden_layers"]
    hidden_size = params["hidden_size"]
    num_heads = params["num_heads"]
    ...
    mask = tf.expand_dims(mask, 2)
    with tf.variable_scope(name):
        with tf.variable_scope("layer_zero"):
            ## @bao
            temp = layer_process(None, x, preproc_actions, prepost_dropout)
            hidden_tensor.append(temp)
            o, w = common_attention.multihead_attention(
                ...
                summaries=False,
                name="encoder_self_attention")
            x = layer_process(None, o, "d", prepost_dropout)
            o = transformer_ffn_layer(layer_process(None, x, preproc_actions, prepost_dropout), params)
            x = layer_process(x, o, postproc_actions, prepost_dropout)
            x = tf.multiply(x, mask)
            ## @bao
...
def output_layer(decoder_output, lang, params):
    ...
    with tf.variable_scope('WordEmbedding', reuse=True):
      ...
      #emb_all = tf.get_variable('C')
      #langs, v_sizes = zip(*params["src_vocab_size"].items())
      #emb_split = tf.split(emb_all, v_sizes)
      #lang_emb = {}
      #for idx, ll in enumerate(langs):
      #  lang_emb[ll] = emb_split[idx]
      #trg_emb = lang_emb[lang]
...
def average_gradients(tower_grads):
    ...
    for grad_and_vars in zip(*tower_grads):
        ...
        ## multi slice index with risk
        #if isinstance(grad, tf.IndexedSlices):
        #    for g_v in grad_and_vars:
        #        average_grads.append(g_v)
        #    continue
...
def add_emb_align(loss, params):
    add_loss = 0.0
    emb_to_cal = []
    for lang in loss:
        with tf.variable_scope('WordEmbedding', reuse=True):
            trg_emb = tf.get_variable('C_%s'%lang)
            emb_to_cal.append(trg_emb[:params['n_emb_align']])
    emb_mean_len = [tf.reduce_mean(tf.abs(emb), 0) for emb in emb_to_cal]
    emb_mean = [tf.reduce_mean(emb, 0) for emb in emb_to_cal]
    emb_std = [tf.sqrt(tf.reduce_mean(tf.square(e - tf.expand_dims(m, 0)), 0)) for e, m in zip(emb_to_cal, emb_mean)]

    emb_mean_std = [(e,s,l) for e, s, l in zip(emb_mean, emb_std, emb_mean_len)]
    n_edge_alpha = 0
    n_edge_beta = 0
    for idx, (mean_1, std_1, mean_len_1) in enumerate(emb_mean_std):
        for mean_2, std_2, mean_len_2 in emb_mean_std[idx+1:]:
            if 'emb_align_alpha' in params and params['emb_align_alpha']:
                add_loss += params['emb_align_alpha'] * tf.sqrt(tf.reduce_sum(
                                                            ((mean_1 - mean_2) / (mean_len_1 + mean_len_2)) ** 2))
                n_edge_alpha += 1
            if 'emb_align_beta' in params and params['emb_align_beta']:
                add_loss += params['emb_align_beta'] * tf.sqrt(tf.reduce_sum(((std_1 - std_2) / (std_1 + std_2)) ** 2))
                n_edge_beta += 1

    n_edge_gama = 0
    ## average sim
    if 'emb_align_gama' in params and params['emb_align_gama']:
        if 'emb_align_gama_nsample' in params and params['emb_align_gama_nsample']:
            tf.logging.info('==== Sampling Emb for Sim Align ====')
            temp = []
            for emb in emb_to_cal:
                rand_idx = tf.random_uniform(shape=[params['emb_align_gama_nsample'], ],
                                             minval=0, maxval=params['n_emb_align'], dtype=tf.int64)
                sampled_emb = tf.gather(emb, rand_idx)
                temp.append(sampled_emb)
            emb_to_cal = temp #warning: emb_to_cal is replaced

        if params['emb_align_gama_type'] == 'cos':
            tf.logging.info('== using gama type cos ==')
            norm_emb_to_cal = []
            for emb in emb_to_cal:
                length = tf.sqrt(tf.reduce_sum(tf.square(emb), 1))
                norm_emb_to_cal.append(emb / tf.expand_dims(length, 1))
            for idx, norm_emb_1 in enumerate(norm_emb_to_cal):
                for norm_emb_2 in norm_emb_to_cal[idx+1:]:
                    emb_loss12 = params['emb_align_gama'] * 0.5 * (1 - tf.reduce_mean(tf.matmul(norm_emb_1, 
                                                                                         norm_emb_2, 
                                                                                         transpose_b=True)))
                    if params['emb_align_gama_reg'] == True:
                        emb_loss11 = params['emb_align_gama'] * 0.5 * (1 - tf.reduce_mean(tf.matmul(norm_emb_1,
                                                                                         norm_emb_1, 
                                                                                         transpose_b=True)))
                        emb_loss22 = params['emb_align_gama'] * 0.5 * (1 - tf.reduce_mean(tf.matmul(norm_emb_2,
                                                                                         norm_emb_2, 
                                                                                         transpose_b=True)))
                        add_loss += 2 * emb_loss12 - emb_loss11 - emb_loss22
                    else:
                        add_loss += emb_loss12
                    n_edge_gama += 1

        elif params['emb_align_gama_type'] == 'l2':
            tf.logging.info('== using gama type l2 ==')
            for idx, emb_1 in enumerate(emb_to_cal):
                for emb_2 in emb_to_cal[idx+1:]:
                    emb1_sqr = tf.expand_dims(tf.reduce_sum(tf.square(emb_1), 1), 1)
                    emb2_sqr = tf.expand_dims(tf.reduce_sum(tf.square(emb_2), 1), 0)
                    sim = tf.matmul(emb_1, emb_2, transpose_b=True)
                    emb12_loss = params['emb_align_gama'] * tf.reduce_mean(tf.sqrt(tf.clip_by_value(emb1_sqr + emb2_sqr - 2 * sim, 1e-6, 1e6)))
                    #emb12_loss = params['emb_align_gama'] * tf.reduce_mean(tf.sqrt(emb1_sqr + emb2_sqr - 2 * sim + 1e-4))

                    if params['emb_align_gama_reg'] == True:
                        sim1 = tf.matmul(emb_1, emb_1, transpose_b=True)
                        emb11_loss = params['emb_align_gama'] * tf.reduce_mean(
                                                                tf.sqrt(tf.clip_by_value(emb1_sqr + tf.transpose(emb1_sqr) - 2 * sim1, 1e-6, 1e6)))
                                                                #tf.sqrt(emb1_sqr + tf.transpose(emb1_sqr) - 2 * sim1 + 1e-4))

                        sim2 = tf.matmul(emb_2, emb_2, transpose_b=True)
                        emb22_loss = params['emb_align_gama'] * tf.reduce_mean(
                                                                tf.sqrt(tf.clip_by_value(tf.transpose(emb2_sqr) + emb2_sqr - 2 * sim2, 1e-6, 1e6)))
                                                                #tf.sqrt(tf.transpose(emb2_sqr) + emb2_sqr - 2 * sim2 + 1e-4))

                        add_loss += 2 * emb12_loss - emb11_loss - emb22_loss
                    else:
                        add_loss += emb12_loss
                    n_edge_gama += 1
        else:
    ...
    tf.logging.info('==== Emb Mean Align add %d edges ===='%n_edge_alpha)
    tf.logging.info('==== Emb Var Align add %d edges ===='%n_edge_beta)
    tf.logging.info('==== Emb Sim Align add %d edges ===='%n_edge_gama)
...
def add_emb_align_identical(loss, params):
    ...
    for i in range(num_lang - 1):
        for j in range(i + 1, num_lang):
            ...
            #for lang in langs[1:]:
            #    overlap = overlap.intersection(vocab_set[lang])
            #overlap = set.intersection(vocab_set[lang] for lang in loss)
            ...
            #for i in range(1, len(emb_to_cal)):
            #    diff = emb_to_cal[i] - emb_to_cal[0]
            #    add_loss += params['emb_align_identical_alpha'] * tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square(diff), -1)), -1)

    #loss['es'] = loss['es'] + add_loss
    #return loss
    #return {lang: loss[lang] + add_loss for lang in loss}
    return loss
...
def add_layer_adv(loss, lang_hidden, lang_mask, params):
    ...
    #return {lang:dis_loss for lang in loss}
    return {lang:loss[lang] + add_loss for lang in loss}
...
def get_loss(features, params, is_train=True):
    ...
    for lang, lang_feature in features.items():
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
            ...
            if params["forward"] is True:
                ...
                ## for predict
                nextword_hidden[lang] = body_output
            ...
            if params["forward"] is True and params["backward"] is True:
                ...
                loss[lang] = loss_b
            ## save layer hidden
            #lang_hidden[lang] = list(hidden_tensor)
    ...
    if is_train and params['n_emb_align']:
        loss = add_emb_align(loss, params)
...
def init_model(model_file):
    ...
    #map_v = {v:v for v in var_to_shape_map}
    for v in all_var:
        v = v.name[:-2]
...
def compute_gradients_split(loss, params):
    ...
    #    else:
    #        var_other.append(v)
    #grad_emb = optimizer_emb.compute_gradients(loss, var_emb)
    #grad_other = optimizer_other.compute_gradients(loss, var_other)
    grad_emb, grad_other = [], []
    ...
    with tf.variable_scope('LMModel') as var_scope:
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            if 'learning_rate_emb_split' in params and params['learning_rate_emb_split']:
                ...
                #train_op = tf.group(train_op_emb, train_op_other)
                ## confused @bao
                #train_op = tf.cond(step < pre_steps, lambda: train_op_other, lambda: tf.group(train_op_emb, train_op_other))
            else:
        ...
        if mode == tf.estimator.ModeKeys.EVAL:
            ...
            #loss = get_loss(features, params_, is_train=False)
            loss = tf.reduce_mean([v for v in loss.values()])
        ...
        if mode == tf.estimator.ModeKeys.PREDICT:
            ...
            atten = tf.concat(atten_tensor, axis=2)
            ## @bao
            tf_hub.add_signature(inputs=features, outputs=h)
            ## predict next word
            import numpy
            langs = nextword_hidden.keys()
            for l in langs:
                if l == 'en':
                    s_lang = l
                else:
                    t_lang = l
            tf.logging.info('reload precal %s_%s'%(s_lang, t_lang))
            precal = numpy.load('./precal_csls/%s_%s.npz'%(s_lang, t_lang))
            precal_topk_emb = {s_lang:precal['s_lang'][None, :], t_lang:precal['t_lang'][None, :]}
            output_nextword = {'input_%s'%lang:features[lang] for lang in features}
            for lang in nextword_hidden:
                for to_lang in nextword_hidden:
                    ## softmax
                    logits = output_layer(nextword_hidden[lang], to_lang, params_)
                    prob = tf.nn.softmax(logits)
                    output_nextword['%s2%s'%(lang, to_lang)] = tf.argmax(prob, axis=-1)

                    ## CSLS
                    #hidden_size = params["hidden_size"]
                    #trg_vocab_size = params["src_vocab_size"][to_lang]
                    #with tf.variable_scope('WordEmbedding', reuse=True):
                    #  trg_emb = tf.get_variable('C_%s'%to_lang)
                    #  norm_trg_emb = trg_emb / tf.sqrt(tf.reduce_sum(trg_emb ** 2, axis=1))[:, None]

                    #hidden_shape = tf.shape(nextword_hidden[lang])[:-1]
                    #hidden = tf.reshape(nextword_hidden[lang], [-1, hidden_size])

                    #norm_hidden = hidden / tf.sqrt(tf.reduce_sum(hidden ** 2, axis=1))[:, None]
                    #csls_score = 0.0
                    #csls_sim = tf.matmul(norm_hidden, norm_trg_emb, transpose_b=True)

                    ##top_v, top_i = tf.nn.top_k(tf.matmul(norm_hidden, norm_trg_emb, transpose_b=True), 6, sorted=False)
                    ##r_x = tf.reduce_mean(top_v[:, 1:], axis=1)[:, None]
                    ##csls_score += csls_sim - r_x
                    #
                    #if to_lang in precal_topk_emb:
                    #    r_y = precal_topk_emb[to_lang]
                    #else:
                    #    pass
                    #    tf.logging.info('Should not enter this')
                    #    exit()
                    #    # cal too heavy
                    #    #top_v, top_i = tf.nn.top_k(tf.matmul(norm_trg_emb, norm_trg_emb, transpose_b=True), 6, sorted=False)
                    #    #r_y = tf.reduce_mean(top_v, axis=1)[None, :]
                    #    #precal_topk_emb[to_lang] = r_y
                    #csls_score += csls_sim  - r_y
                    #csls_score = tf.reshape(csls_score, tf.concat([hidden_shape, [trg_vocab_size]], 0))
                    #output_nextword['%s2%s'%(lang, to_lang)] = tf.argmax(csls_score, axis=-1)

            #return tf.estimator.EstimatorSpec(mode=mode, predictions={'hidden': h, 'input':features, 'atten':atten})
            return tf.estimator.EstimatorSpec(mode=mode, predictions=output_nextword)
